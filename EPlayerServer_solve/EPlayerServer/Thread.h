...
public:
	...
	int Start() {
		...
		if (ret != 0)return -2;
		//ret = pthread_attr_setscope(&attr,PTHREAD_SCOPE_PROCESS);//进程内竞争
		//if (ret != 0)return -3;
		ret = pthread_create(&m_thread, &attr,&CThread::ThreadEntry, this);//创建线程
		if (ret != 0)return -4;
		m_mapThread[m_thread] = this;//记录线程ID和线程对象之间的映射关系
	...
	int Pause() {
		...
		m_bpaused = true;
		int ret = pthread_kill(m_thread,SIGUSR1);//结束线程
		if (ret != 0) {
			m_bpaused = false;
			return -2;
		}
