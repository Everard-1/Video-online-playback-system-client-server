...
public:
	...
	virtual int BusinessProcess(CProcess* proc) {
		...
		KeyValue args;
		args["host"] = "192.168.17.152";
		args["user"] = "root";
		args["password"] = "StrongPass123!";
		args["port"] = 3306;
		args["db"] = "edoyun";
...
private:
	...
	int HttpParser(const Buffer& data) {
		CHttpParser parser;
		size_t size = parser.Parser(data);//把 data 解析成 请求方法 + URL + 参数
		...
		if (parser.Method() == HTTP_GET) {
			//get处理
			UrlParser url("https://192.168.17.152" + parser.Url());
			int ret = url.Parser();
			if (ret != 0) {
				TRACEE("ret = %d url[%s]", ret, "https://192.168.17.152" + parser.Url());
				return -2;
			}
			Buffer uri = url.Uri();
