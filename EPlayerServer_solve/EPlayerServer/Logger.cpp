...
LogInfo::LogInfo(const char* file, int line, const char* func, pid_t pid, pthread_t tid, int level)//自己主动发的 流式的日志
{
	bAuto = true;
	const char sLevel[][8] = {
		"INFO","DEBUG","WARNING","ERROR","FATAL"
...
LogInfo::LogInfo(const char* file, int line, const char* func, pid_t pid, pthread_t tid, int level, void* pData, size_t nSize)
{
	...
	for (; i < nSize; i++) {
		...
		if (0 == ((i + 1) % 16)) {
			...
			//	if ((Data[j] & 0xFF) > 31 && ((Data[j] & 0xFF) < 0x7F)) {//可显示字符范围
			//		m_buf += Data[i];
			//	}
			//	else {
			//		m_buf += ".";
	...
	if (k != 0) {
		...
		for (size_t j = i - k; j <= i; j++) {
			if ((Data[i] & 0xFF) > 31 && ((Data[j] & 0xFF) < 0x7F)) {//可显示字符范围
				m_buf += Data[i];
			}
			else {
				m_buf += '.';
