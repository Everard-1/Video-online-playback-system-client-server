...
{
	if (!m_bInit) return -1;//没连接报错
	int ret = mysql_real_query(&m_db, sql, sql.size());//执行 SQL 查询
	if (ret != 0) {
		printf("%s %d %s\n", __FUNCTION__, mysql_errno(&m_db), mysql_error(&m_db));
		TRACEE("%d %s", mysql_errno(&m_db), mysql_error(&m_db));
		return -2;
...
int CMysqlClient::StartTransaction()
{
	...
	if (ret != 0) {
		printf("%s %d %s", __FUNCTION__, mysql_errno(&m_db), mysql_error(&m_db));
		TRACEE("%d %s", mysql_errno(&m_db), mysql_error(&m_db));
		return -2;
	}
	return 0;
...
Buffer _mysql_table_::Query(const Buffer& condition)
{
	...
	for (size_t i = 0; i < FieldDefine.size(); i++) {
		if (i > 0) sql += ',';
		sql += '`' + FieldDefine[i]->Name + "` ";//双引号改为`号
	}
