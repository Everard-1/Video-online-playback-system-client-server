    ...
    , helper(this)
{
    ...
    //当前没有播放任何视频
    current_play = -1;  //当前播放的视频是 右侧视频列表的第几个
    //按钮背景透明
...
void Widget::timerEvent(QTimerEvent* event)
{
    ...
    {
        check_thread[GetTickCount()%32].start();    //随机启动一个线程，检测是否正常登录
        //TODO:服务器保持活跃
    }
    else if(event->timerId() == timesID) //1s
    {
        timesCount++;
        if(timesCount > 5)      //播放倍率的窗口，因为是悬浮的。
        {
            timesCount = 0;
            ui->time0_5->setVisible(false);
            ui->time1->setVisible(false);
...
{
    if(watched == ui->horizontalSlider)     //进度条event
    {
