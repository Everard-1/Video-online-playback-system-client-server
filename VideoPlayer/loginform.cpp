...
//const char* HOST = "http://code.edoyun.com:6191";
const char* HOST = "http://192.168.17.152:9999";
    ...
    auto_login_id(-1)//定时器id
{
    ...
    ui->setupUi(this);  //无头窗口，没有标题栏相关内容
    ...
    ui->forget->installEventFilter(this);   //忘记密码的控件
...
bool LoginForm::eventFilter(QObject* watched, QEvent* event)
{
    ...
    {
        if(event->type() == QEvent::FocusIn)    //有焦点，样式修改。
        {
            ui->pwdEdit->setStyleSheet("color: rgb(251, 251, 251);background-color: transparent;");
        }
        else if(event->type() == QEvent::FocusOut)  //没有焦点，样式修改。
        {
...
void LoginForm::on_remberPwd_stateChanged(int state)
{
    //记住密码状态改变
    //qDebug() << __FILE__ << "(" << __LINE__ << "):";
    record->config()["remember"] = state == Qt::Checked;    //是否等于 勾选状态， == 布尔值
    if(state == Qt::Checked)
    {
        //qDebug() << __FILE__ << "(" << __LINE__ << "):";
    }
